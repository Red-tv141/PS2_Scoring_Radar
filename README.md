...
1. 
**Firewalls:** Immediately discards system, IO, and dependency-critical calls. 


2. 
**Thunk Resolution:** Manually resolves bare `j` wrappers that Ghidra might misinterpret, ensuring accurate analysis of the true target. 


3. 
**Base Scoring:** Evaluates the JAL across 5 layers (DNA type, Kill Zone depth, Address safety, Tree depth, and FrameRate Nexus connection). 


4. 
**Pattern Hunting:** Modifies the score based on advanced heuristic modules (Euler, Kinematic, etc.). 


5. 
**Output:** Generates a ranked `.pnach` file formatted for immediate testing in PCSX2. 
